...
sim_SEQ <-  function(N=200, dist="bern", K=1, par1 = c(0,0), par2 = NULL, measure="sd",burnin=3, ar="WMW",nsim=10^4, alpha=0.05, one.sided = FALSE){
  
  if(K==1){
    out = matrix(nrow = nsim, ncol = 7)
    for(i in 1:nsim){
      if(i==1){
        out[i,] = two_arm_seq(N=N, dist=dist,  par1 =par1, par2 = par2, measure=measure, burnin = burnin,ar=ar, first=FALSE)
      } else {
        out[i,] = two_arm_seq(N=N, dist=dist,  par1 =par1, par2 = par2, measure=measure, burnin = burnin,ar=ar)
      }
...
two_arm_seq <- function(N=200, dist="bern",  par1 = c(0,0), par2 = NULL, measure="sd", burnin=3, ar="WMW", Z_corrected=FALSE, first=FALSE){
...
sim_SEQ(N=200, dist="bern", K=1, par1 = c(0.994,0.991), burnin=10, ar="Neyman",nsim=10^1, measure = "lrr")
